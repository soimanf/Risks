...
\title{\boldmath Blockchain technology and the crypto-market's risks: A literature survey \footnote{A former version of this paper was entitled "Blockchain technology and cryptomarket:vulnerabilities and risk assessment"

This paper has been presented at the 12th International Multi-Conference on Complexity, Informatics and Cybernetics (IMCIC 2021) in Orlando, USA, and at Workshop Blockchains \& Cryptomonnaies 2021 in Grenoble, France.}}
...
\begin{abstract}
This paper provides a literature survey on the vulnerabilities and risks of Blockchain technology and the crypto-market. Since their creation, the crypto-market and Blockchain technology are still very much challenged and far from mainstream adoption. Thus, we propose a detailed literature survey focusing on the relationship between technological characteristics and financial risks. Furthermore, to complete this study, we propose ways to determine the likelihood of technological vulnerabilities triggering financial risks. We find a significant relationship between Blockchain attacks and crypto-assets' volatility, illustrating the relationship between technological vulnerabilities and financial risk. Our contributions are threefold. First, we perform a literature survey comprising the crypto-market’s risks. Secondly, we show a link between technological risks and financial ones. Thirdly, we provide empirical results showing that bitcoin’s price stability is disturbed by technological vulnerabilities.
\\
...
same idea is sustained in the surveys conducted by Deloitte and Underscore companies. While assessing the Blockchain adoption, Deloitte found out that some of the main barriers are: technological complexity, regulatory issues, lack of in-house skills and understanding, security threats, and the uncertain
...
vulnerabilities. The crypto-assets’ volatility and the numerous
...
literature survey, we complete this work with a short data analysis. In line with the statements made in the literature review, we show that bitcoin’s price instability (financial risk) can be triggered by attacks targeting the crypto-market (technological vulnerability) and investor's attention (triggered by technological vulnerabilities).
\\
...
Here, we arrange the crypto-market threats in accordance with their
...
stake owners try to decrease the value of the crypto-asset. Indeed, the
...
(January 2019) and Bitcoin Cash (May 2019) \citep{Beigel2019}. The size of the Blockchain network very much influences the difficulty of executing an attack. Table \ref{attacks} shows how expensive it is to perform a majority attack, depending on the crypto-asset. These costs are computed taking into account the expenses incurred in the mining process, namely the network hash rate \& the Nicehash cost in
BTC/per hour (rented PC power). These values can change every minute,
as the crypto-assets' prices have a strong influence \citep{Crypto51.app2020}.


\begin{table}[h!]
\centering
\caption{\textbf{PoW 51\% attack cost for the top 7 crypto-assets.}}
\begin{tabular}{||c c c ||}
 \hline
...
crypto-assets \citep{Crothers2021}.
...
{\multirow{7}{*}{\rotatebox[origin=r]{90}{\cellcolor[HTML]{FFFFFF}\textbf{Consensus level attack}}}} &
 ...
 {\cellcolor[HTML]{EFEFEF}\textit{· Manipulates the system by entering
  ...
  {{\cellcolor[HTML]{EFEFEF}\textit{Blockchains using PoS (over 350 crypto-assets\floatfootnote{The total number of crypto-assets is over 2200 \citep{CryptoSlate.com2020}}) source:\citep{CryptoSlate.com2020}}}}
 \\
&
  {\cellcolor[HTML]{FFFFFF}} &
  {\cellcolor[HTML]{FFFFFF}\textit{·  Manipulates the system}} &
  {\cellcolor[HTML]{FFFFFF}\textit{Blockchains using PoW consensus (over 500 crypto-assets);}} \\
 &
  {\cellcolor[HTML]{FFFFFF}} &
  {\cellcolor[HTML]{FFFFFF}\textit{·  Monopolized consensus process}} &
  {\cellcolor[HTML]{FFFFFF}\textit{Blockchains using PoS (over 350 crypto-assets)source: \citep{CryptoSlate.com2020}}} \\
 &
  {\cellcolor[HTML]{FFFFFF}} &
...
{\multirow{2}{*}{\cellcolor[HTML]{FFFFFF}\spheading[50pt]{\centering\textbf{Smart
 ...
 &
  ...
  {\cellcolor[HTML]{EFEFEF}\textit{Blockchains   supporting smart contracts (over 50 crypto-assets) Source:\citep{CryptoSlate.com2020}}} \\
 &
  {\cellcolor[HTML]{FFFFFF}\textbf{Smart contract attack}} &
  {\cellcolor[HTML]{FFFFFF}\textit{Misleads the technology’s application}} &
  {\cellcolor[HTML]{FFFFFF}\textit{Blockchains supporting smart contracts (over 50 crypto-assets) Source:\citep{CryptoSlate.com2020}}} \\ \hline
\end{tabulary}
%}
\end{sidewaystable}
...
\textbf{Liquidity risk.} A market is said to be liquid if an agent can rapidly make some significant trades without creating an important change in the price (small market impact). In other words, in a liquid market, transactions will likely not change the price, but new information will be smoothly incorporated. On the other hand, an illiquid market (often linked to an inefficient market) will reflect in large volatility in prices (hence a higher probability of an unfair price), a lower number of investors, and lower chances to transact/trade. Liquidity risk can be split into three categories: assets liquidity (refers to the interaction between sellers and buyers on the platform and the asset availability on exchanges), exchange liquidity (refers to the interaction between makers and takers concerning the assets’ and the orders’ supply) and market liquidity (encompasses the first two) \citep{Crowell2020}. According to \cite{Corbet2019}, liquidity risk is also highly correlated with the events concerning cyber-attacks or regulatory issues as a response to human behavior and investors’ attitude towards this market. At the same time, the most debated factors explaining liquidity in the crypto-market are the price, trading volume,
 capitalization, fees, hash value (for PoW crypto-assets), and the size of the network \citep{Koutmos2018}.

It is important to mention the fact that liquidity is different from one crypto-asset to another (the well-established ones are more liquid \citep{Wei2018, Koutmos2020}), as well as from one exchange platform to another. Despite the many benefits associated with liquidity, illiquid environments can also present some advantages, especially for the traders on this market, which can benefit from arbitrage opportunities and purchases at discounts \citep{Crowell2020}.


\underline{Likelihood:} Analyzed from the crypto-assets’ (crypto-assets that claim to be ‘money’) perspective, this risk would translate into an impossibility to be transformed in cash. That being said, one of the principal roles of money (being a medium of exchange) has just failed \citep{Greene2018}. There are many triggers behind crypto-assets illiquidity, among which: token supply algorithm, investors’ behavior, available supply, asset usage, fees, exchange
...
\subsection{Crypto events impact crypto-assets' price }
\label{sect3.2}

In line with the literature survey done in the previous section, here we are going to provide an example of how financial risk is linked to technological vulnerabilities. More specifically, we assess if bitcoin’s volatility is affected by events targeting the crypto-market. Some preliminary work on this problem has already been done by \cite{Corbet2020, Caporale2021, Grobys2021}.  \cite{Corbet2020} proved that (17) hackings that took place between 2017 and 2018 had affected the volatility and cross-correlation for the top 8 crypto-assets, while \cite{Grobys2021} showed how the (29) cyber-attacks performed on bitcoin during the 2013–2017 period affected BTC and ETH returns.  \cite{Caporale2021} demonstrated how (4693) cyber-attacks targeting not only the crypto-market and happening between 2015 to 2020 created spillovers and contagion effects among the top three crypto-assets. \\

In our analysis, we are using a sample of (53) events, which cover both the early times of bitcoin (2011-2013) as well as the hype period in 2018. \cite{Corbet2019} showed that among many factors, news related to cyber-attacks have an important impact on the price movement of crypto-assets. As of January 2022, the amounts lost during our events (2011-2018) correspond to a 39 billion Eur (945,066 BTC) monetary equivalent. Given the extent of the losses incurred, it would be interesting to investigate if they impact the market. Therefore, in addition to what was already shown, more specifically, that cyber-attacks events impact the price of crypto-assets, we want to take it a little further and see if the market is sensitive to the amounts lost. \\
...
\begin{table}[h]
	...
	\label{tab:descriptiveStatistics}
		\begin{tabular}{lrrr}
			...
			\bottomrule
		\end{tabular}
			\\ \footnotesize{\textit{The table summarizes the descriptive statistics of all variables for the sample period.}}
	}
	\end{table}
...
\begin{itemize}
\item Crypto-assets represent a type of virtual currencies; built on open-source software code, they exist and operate just in the online environment. This makes them the target of cyber-attacks that try to exploit any possible vulnerability of this technology.
\item Crypto-assets’ users need cryptographic keys in order to
...
crypto-asset world made them the perfect tool for the black markets;
these ones are also the few places accepting crypto-assets as payment.
...
\subsection{Negative investor attention impact on crypto-assets' price }

In this section we investigate the relationship between the returns of bitcoin, the price volatility and Google Trends. Using a set of keywords related to bitcoin, Blockchain technology and crypto-assets, we assess if the investor’s attention can be captured by the Google Search Volume (GSV), and if it impacts bitcoin’s performance. This analysis is complementing the later one. When the crypto-market becomes the target of an attack or other type of significant event that involves financial losses, these scenarios usually attract attention. Under the close scrutiny of the public eye and combined with the negative image set by risky events, the crypto-assets' prices become often unstable. In this section, we investigate the impact of negative investor's attention triggered by uncertain events such as those studied in \ref{sect3.2} analysis. \\

Despite their relatively short history, crypto-assets have attracted much societal attention which has been reflected in the large price fluctuations \citep{Hasso2019}. \cite{Griffin2020} state that given the excessive attention and rapid market capitalization growth, factors which are common in the characterization of financial bubbles, there is no surprise that crypto-assets are subject to price manipulation, misinformation, and speculation. Several studies have identified anomalies in the crypto-assets’ returns \citep{Bariviera2017, Chaim2019, Kristoufek2013, Urquhart2016}, which lead to further questions about the crypto-assets’ nature \citep{Baur2018b, Dyhrberg2016, Kruckeberg2019}, the investor’s behavior \citep{Benedetti2021, Hasso2019, Kallinterakis2019}, and financial bubbles \citep{Chaim2019, Cheah2015, Corbet2019, Fry2016, Kyriazis2020}. Compared to other asset classes, crypto-assets seem to be more volatile \citep{Hardle2020, Urquhart2019}. According to \citep{Lucey2022}, the risky features of crypto-assets makes them particularly attractive to ‘amateur’ investors. Hence, the presence of such ‘noise’ traders could be a justification for the high volatility present in the crypto-market and implicitly the significant deviation from the fundamental value \citep{DeLong1990}. \\

Investor’s attention, associated with the under- and over-reaction of market participants, has been long considered an explanation for security mispricing \citep{Andrei2015, Baker2007, Shiller2000}. The pioneers of behavioral finance, \cite{Tversky1974} argue that in situations of uncertainty, people tend to ignore the laws of probability and discern events irrationally. In the crypto-market context, this would translate in situations such as investors misinterpret certain episodes of exponential growth in the crypto-assets’ prices and chose to put their capital at risk before considering a scenario such as financial bubble. With their findings, \cite{Lin2020a} confirms \cite{Tversky1974}’s theory, such as crypto-assets’ performance drives future investor’s attention. This is especially important because investment preferences and selection choices are determined by what news or information catches first the attention of investors \citep{Barber2008a}. That being said, if for instance crypto-assets perform well and that stimulates the investor’s attention, the chances that this attention attracts more ‘amateurs’ creating noise and irrational price fluctuations, increases \citep{Lucey2022}. \cite{AlGuindy2021} states that whether investor’s attention has a positive or negative influence on the crypto-assets’ prices, it is still imprecise.\\

We follow \cite{Liu2021} and use Google Search Volume (GSV) as our measure for negative investor’s attention. In their paper, the negative investor attention measures the change in market attention (online search frequency) coming from  risky events associated with the crypto-market. The rationale behind this approach is the following. In this Internet and smart technology era that we live in, our attention is constantly challenged by the overabundance of information available. Google engine accounts for approximately 90\% of the total online searches around the world\footnote{See report about market share of leading search engines: https://www.statista.com/statistics/216573/worldwide-market-share-of-search-engines/} \citep{Smales2022}. As online searches represent a direct measure of attention \citep{Barber2008, Lin2020a, Nasir2019} and anyone searching for information about a particular crypto-asset will certainly do it online, we believe that GSV is the appropriate proxy for investor attention. In their study, \cite{Liu2021} assess if negative investor’s attention impacts bitcoin’s returns. The results show that bitcoin is negatively and significantly driven by the attention proxy. \\

Similar to the previous data analysis section, we will look only at bitcoin, the leader of the crypto-market. In accordance with the literature we establish the following hypotheses:\\

\textbf{H1: Bitcoin’s volatility is positively impacted by negative investor attention.\\
H2: The returns of bitcoin are negatively impacted by negative investor attention.}\\

We construct our proxy for negative investor’s attention using Google (worldwide) searches for the following phrases “cryptocurrency attack”, “cryptocurrency hack”, “cryptocurrency risk”, “bitcoin attack”, “bitcoin hack”, “bitcoin risk”, “bitcoin exploit”, “blockchain attack”, “blockchain hack”, “blockchain risk”, “zero day”. The selection of these phrases has been made considering that by negative investor’s attention, we refer to the attention received by the crypto-market, when a crypto-asset was the target of a negative event\footnote{By negative events we refer to attacks, hacks and any other similar vulnerability that could have a negative impact on the crypto-assets’ prices and bring uncertainty in this market.}. In our Google search we work with the term ‘cryptocurrency’ instead of ‘crypto-asset’, simply because out of the scientific and legal world\footnote{ The Banque de France’s view on bitcoin and other crypto-assets: https://www.youtube.com/watch?v=Dqaw9-NTd3c}, ‘cryptocurrency’ is the most common word to use when referring to the crypto-market. Furthermore, we consider that the investors interested in the crypto-market could be categorized as specialists in Blockchain technology and non-specialists. This information is relevant, as someone who is a specialist or knowledgeable in Blockchain technology, would rather use words such as ‘hack’, ‘exploit’ or ‘zero-day’\footnote{"Zero-day" is a phrase that refers to the recent discovery of a serious security computer-software related vulnerability. As its name suggests, the developers have zero day to fix it, before it could be exploited by attackers.} in searching for information. Therefore, we consider a variety of keywords that aim to capture the attention from both groups. We retrieved from Google engine the data used to proxy  negative investor attention. Data spans from April 2013 – April 2022, with a monthly frequency (given the long period studied, Google automatically generated data at monthly frequency). For bitcoin, we used the data from Thomson Reuters Eikon database as in the previous analysis. We compute monthly returns and volatility (standard deviation) for the period April 2013 – April 2022. 
Table \ref{tab:descriptstat} displays the descriptive statistics of all variables used.
...
\begin{table}[h]
	\centering
		{
		\caption{Descriptive Statistics of all variables}
	\label{tab:descriptstat}
	\resizebox{\textwidth}{!}{ 
		\begin{tabular}{llllllll}
 & \cellcolor[HTML]{FFFFFF}\textbf{Panel A} & \cellcolor[HTML]{FFFFFF}\textit{\textbf{Mean}} & \cellcolor[HTML]{FFFFFF}\textit{\textbf{SD}} & \cellcolor[HTML]{FFFFFF}\textit{\textbf{Sharpe}} & \cellcolor[HTML]{FFFFFF}\textit{\textbf{Skewness}} & \cellcolor[HTML]{FFFFFF}\textit{\textbf{Kurtosis}} & \cellcolor[HTML]{FFFFFF}\textit{\textbf{DW}} \\ \cline{2-8} 
 & \multicolumn{1}{l|}{\cellcolor[HTML]{FFFFFF}\textit{\textbf{BTC Rt}}} & 6.931 & 29.127 & 0.238 & 1.603 & 7.608 & 1.483 \\ \cline{2-8} 
 & \multicolumn{1}{l|}{\cellcolor[HTML]{FFFFFF}\textit{\textbf{Volatility}}} & 4.747 & 3.217 & 1.476 & 2.534 & 9.616 & 0.396 \\ \cline{2-8} 
 &  &  &  &  &  &  &  \\
\rowcolor[HTML]{FFFFFF} 
\textbf{Panel B} & \textit{\textbf{Mean}} & \textit{\textbf{SD}} & \textit{\textbf{Max}} & \textit{\textbf{Min}} & \textit{\textbf{Skewness}} & \textit{\textbf{Kurtosis}} & \textit{\textbf{DW}} \\ \hline
\multicolumn{1}{l|}{\cellcolor[HTML]{FFFFFF}\textit{\textbf{crypto\_attack}}} & 18.872 & 22.185 & 100.000 & 0.000 & 1.596 & 2.332 & 0.289 \\ \hline
\multicolumn{1}{l|}{\cellcolor[HTML]{FFFFFF}\textit{\textbf{crypto\_hack}}} & 18.872 & 22.185 & 100.000 & 0.000 & 1.596 & 2.332 & 0.289 \\ \hline
\multicolumn{1}{l|}{\cellcolor[HTML]{FFFFFF}\textit{\textbf{crypto\_risk}}} & 19.110 & 23.838 & 100.000 & 0.000 & 1.363 & 0.952 & 0.105 \\ \hline
\multicolumn{1}{l|}{\cellcolor[HTML]{FFFFFF}\textit{\textbf{bitcoin\_attack}}} & 20.679 & 16.367 & 100.000 & 2.000 & 1.893 & 4.748 & 0.174 \\ \hline
\multicolumn{1}{l|}{\cellcolor[HTML]{FFFFFF}\textit{\textbf{bitcoin\_hack}}} & 20.679 & 16.367 & 100.000 & 2.000 & 1.893 & 4.748 & 0.174 \\ \hline
\multicolumn{1}{l|}{\cellcolor[HTML]{FFFFFF}\textit{\textbf{bitcoin\_risk}}} & 13.688 & 14.438 & 100.000 & 1.000 & 2.820 & 11.218 & 0.180 \\ \hline
\multicolumn{1}{l|}{\cellcolor[HTML]{FFFFFF}\textit{\textbf{blockchain\_attack}}} & 27.037 & 24.173 & 100.000 & 0.000 & 0.639 & -0.249 & 0.111 \\ \hline
\multicolumn{1}{l|}{\cellcolor[HTML]{FFFFFF}\textit{\textbf{blockchain\_hack}}} & 27.037 & 24.173 & 100.000 & 0.000 & 0.639 & -0.249 & 0.111 \\ \hline
\multicolumn{1}{l|}{\cellcolor[HTML]{FFFFFF}\textit{\textbf{blockchain\_risk}}} & 37.339 & 29.698 & 100.000 & 0.000 & 0.197 & -1.165 & 0.080 \\ \hline
\multicolumn{1}{c|}{\cellcolor[HTML]{FFFFFF}\textit{\textbf{zero-day}}} & 28.721 & 14.303 & 100.000 & 9.000 & 2.321 & 7.610 & 0.261 \\ \hline
\multicolumn{1}{l|}{\cellcolor[HTML]{FFFFFF}\textit{\textbf{bitcoin\_exploit}}} & 16.505 & 13.398 & 100.000 & 0.000 & 2.662 & 12.643 & 0.546 \\ \hline
\end{tabular}}
			\\ \footnotesize{\textit{The table summarizes the descriptive statistics of all variables for the sample period. DW is the abbreviation for Durbin-Watson test. In panel A we show the statistical properties of bitcoin returns and volatility.\\			
Panel B reports the summary statistics for the proxies used to measure negative investor’s attention: google searches for “cryptocurrency attack/hack/risk”, ”bitcoin attack/hack/risk/exploit”, “blockchain attack/hack/risk” and "zero day". 
Data spans from April 2013 – April 2022, with a monthly frequency.}	}
}	\end{table}
...
$$ Rt_{BTC}(t)= \alpha + \sum\limits_{(j=0)}^3   \beta_j *  google\_search(t-j)+ \varepsilon. $$

Where $Rt_{BTC}(t)$ is the monthly return of bitcoin at time t, $\beta_j$ is the coefficient related to lag j, and $ google\_search(t-j)$ represents the lagged google search trend. Results are reported in table \ref{tab:negret}.

\begin{table}[h]
	\centering
		{
		\caption{The impact of negative investor’s attention on bitcoin returns}
	\label{tab:negret}
	\resizebox{\textwidth}{!}{ 
		\begin{tabular}{llllllll}
 & \textit{\textbf{Constant}} & \textit{\textbf{T}} & \textit{\textbf{T-1}} & \textit{\textbf{T-2}} & \textit{\textbf{T-3}} & \textit{\textbf{$R^2$}} & \textit{\textbf{google search}} \\ \hline
\multicolumn{1}{l|}{} & \begin{tabular}[c]{@{}l@{}}0.019\\ (0.5)\end{tabular} & \textbf{\begin{tabular}[c]{@{}l@{}}0.092**\\    (2.143)\end{tabular}} & \textbf{\begin{tabular}[c]{@{}l@{}}0.126***\\ (2.824)\end{tabular}} & \textbf{\begin{tabular}[c]{@{}l@{}}0.097**\\ (2.076)\end{tabular}} & \begin{tabular}[c]{@{}l@{}}0.002 \\ (0.053)\end{tabular} & 23.88 & cryptocurrency\_attack \\
\multicolumn{1}{l|}{} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}0.043\\ (1.151)\end{tabular} & \cellcolor[HTML]{EFEFEF}\textbf{\begin{tabular}[c]{@{}l@{}}0.060**\\    (2.5)\end{tabular}} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}0.027\\ (1.116)\end{tabular} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}0.022 \\ (0.884)\end{tabular} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}0.011 \\ (0.451)\end{tabular} & \cellcolor[HTML]{EFEFEF}13.86 & \cellcolor[HTML]{EFEFEF}cryptocurrency\_hack \\
\multicolumn{1}{l|}{} & \begin{tabular}[c]{@{}l@{}}0.009\\ (0.241)\end{tabular} & \textbf{\begin{tabular}[c]{@{}l@{}}0.204***\\  (3.437)\end{tabular}} & \textbf{\begin{tabular}[c]{@{}l@{}}0.149**\\ (2.443)\end{tabular}} & \begin{tabular}[c]{@{}l@{}}0.062 \\ (0.955)\end{tabular} & \begin{tabular}[c]{@{}l@{}}0.074 \\ (1.152)\end{tabular} & 28.49 & cryptocurrency\_risk \\
\multicolumn{1}{l|}{} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}0.041\\ (1.162)\end{tabular} & \cellcolor[HTML]{EFEFEF}\textbf{\begin{tabular}[c]{@{}l@{}}0.226***\\ (3.828)\end{tabular}} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}0.057\\ (0.966)\end{tabular} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}-0.013 \\ (-0.217)\end{tabular} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}-0.004\\ (-0.072)\end{tabular} & \cellcolor[HTML]{EFEFEF}25.61 & \cellcolor[HTML]{EFEFEF}bitcoin\_attack \\
\multicolumn{1}{l|}{} & \begin{tabular}[c]{@{}l@{}}0.039\\ (1.147)\end{tabular} & \textbf{\begin{tabular}[c]{@{}l@{}}0.247***\\ (3.239)\end{tabular}} & \textbf{\begin{tabular}[c]{@{}l@{}}0.145*\\ (1.836)\end{tabular}} & \textbf{\begin{tabular}[c]{@{}l@{}}0.185**\\  (2.309)\end{tabular}} & \begin{tabular}[c]{@{}l@{}}0.056\\ (0.724)\end{tabular} & 22.81 & bitcoin\_hack \\
\multicolumn{1}{l|}{} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}0.029\\ (0.803)\end{tabular} & \cellcolor[HTML]{EFEFEF}\textbf{\begin{tabular}[c]{@{}l@{}}0.229***\\ (3.497)\end{tabular}} & \cellcolor[HTML]{EFEFEF}\textbf{\begin{tabular}[c]{@{}l@{}}0.146**\\ (2.159)\end{tabular}} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}0.026\\ (0.386)\end{tabular} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}0.006\\ (0.09)\end{tabular} & \cellcolor[HTML]{EFEFEF}26.52 & \cellcolor[HTML]{EFEFEF}bitcoin\_risk \\
\multicolumn{1}{l|}{} & \textbf{\begin{tabular}[c]{@{}l@{}}0.102*\\ (2.013)\end{tabular}} & \begin{tabular}[c]{@{}l@{}}-0.017\\ (-0.609)\end{tabular} & \begin{tabular}[c]{@{}l@{}}0.000\\ (0.014)\end{tabular} & \begin{tabular}[c]{@{}l@{}}-0.016\\ (-0.538)\end{tabular} & \begin{tabular}[c]{@{}l@{}}-0.02\\ (-0.723)\end{tabular} & 2.39 & bitcoin\_exploit \\
\multicolumn{1}{l|}{} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}0.065\\ (1.582)\end{tabular} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}-0.055\\ (-0.519)\end{tabular} & \cellcolor[HTML]{EFEFEF}\textbf{\begin{tabular}[c]{@{}l@{}}0.222**\\ (2.027)\end{tabular}} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}-0.037\\ (-0.335)\end{tabular} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}-0.010\\ (-0.089)\end{tabular} & \cellcolor[HTML]{EFEFEF}12.21 & \cellcolor[HTML]{EFEFEF}blockchain\_attack \\
\multicolumn{1}{l|}{} & \begin{tabular}[c]{@{}l@{}}0.027\\ (0.722)\end{tabular} & \textbf{\begin{tabular}[c]{@{}l@{}}0.361***\\ (2.735)\end{tabular}} & \textbf{\begin{tabular}[c]{@{}l@{}}0.351**\\  (2.381)\end{tabular}} & \begin{tabular}[c]{@{}l@{}}0.207\\ (1.406)\end{tabular} & \begin{tabular}[c]{@{}l@{}}0.186\\ (1.386)\end{tabular} & 17.97 & blockchain\_hack \\
\multicolumn{1}{l|}{} & \cellcolor[HTML]{EFEFEF}\textbf{\begin{tabular}[c]{@{}l@{}}0.067*\\ (1.774)\end{tabular}} & \cellcolor[HTML]{EFEFEF}\textbf{\begin{tabular}[c]{@{}l@{}}0.161*\\ (1.784)\end{tabular}} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}-0.067\\ (-0.752)\end{tabular} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}0.047\\ (0.423)\end{tabular} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}0.007\\ (0.062)\end{tabular} & \cellcolor[HTML]{EFEFEF}9.58 & \cellcolor[HTML]{EFEFEF}blockchain\_risk \\
\multicolumn{1}{l|}{\multirow{-11}{*}{\textit{\textbf{Rt BTC}}}} & \textbf{\begin{tabular}[c]{@{}l@{}}0.123***\\ (3.064)\end{tabular}} & \begin{tabular}[c]{@{}l@{}}-0.026\\ (-0.402)\end{tabular} & \textbf{\begin{tabular}[c]{@{}l@{}}-0.143**\\ (-2.041)\end{tabular}} & \begin{tabular}[c]{@{}l@{}}-0.112\\ (-1.592)\end{tabular} & \begin{tabular}[c]{@{}l@{}}-0.105\\ (-1.624)\end{tabular} & 12.92 & Zero-day \\ \hline
\end{tabular}}
			\\ \footnotesize{\textit{Here we assess the impact of negative investor’s attention, proxied by google search terms “cryptocurrency attack/hack /risk”, ”bitcoin attack/hack/risk/", “blockchain attack/hack/risk” and "zero day". We regressed bitcoin's monthly returns against the google search data without lag and lagged from one up to three-months horizons. Rt stands for monthly returns. For each regression, we had 50 observations. *** denotes significance levels based on the respective p-value (*:10\%, **:5\%, and ***:1\%). The standard t-statistic value is shown in parentheses.}	}
}	\end{table}


$$ \sigma_{BTC}(t)= \alpha + \sum\limits_{(j=0)}^3   \beta_j *  google\_search(t-j)+ \varepsilon. $$

Where $\sigma_{BTC}(t)$ is the monthly volatility of bitcoin at time t, $\beta_j$ is the coefficient related to lag j, and $ google\_search(t-j)$ represents the lagged google search trend. Results are reported in table \ref{tab:negvol}.


\begin{table}[h]
	\centering
		{
		\caption{The impact of negative investor’s attention on bitcoin volatility}
	\label{tab:negvol}
	\resizebox{\textwidth}{!}{ 
		\begin{tabular}{llllllll}
 & \textit{\textbf{Constant}} & \textit{\textbf{T}} & \textit{\textbf{T-1}} & \textit{\textbf{T-2}} & \textit{\textbf{T-3}} & \textit{\textbf{$R^2$}} & \textit{\textbf{google search}} \\ \hline
\multicolumn{1}{l|}{} & \textbf{\begin{tabular}[c]{@{}l@{}}0.044***\\ (12.525)\end{tabular}} & \begin{tabular}[c]{@{}l@{}}0.004\\ (1.04)\end{tabular} & \begin{tabular}[c]{@{}l@{}}0.000\\  (0.081)\end{tabular} & \begin{tabular}[c]{@{}l@{}}-0.003\\ (-0.763)\end{tabular} & \begin{tabular}[c]{@{}l@{}}0.004\\ (0.918)\end{tabular} & 5.46 & cryptocurrency\_attack \\
\multicolumn{1}{l|}{} & \cellcolor[HTML]{EFEFEF}\textbf{\begin{tabular}[c]{@{}l@{}}0.046***\\ (13.905)\end{tabular}} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}0.001\\ (0.235)\end{tabular} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}-0.002\\ (-0.718)\end{tabular} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}0.001\\ (0.527)\end{tabular} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}-0.003\\ (-1.254)\end{tabular} & \cellcolor[HTML]{EFEFEF}5.69 & \cellcolor[HTML]{EFEFEF}cryptocurrency\_hack \\
\multicolumn{1}{l|}{} & \textbf{\begin{tabular}[c]{@{}l@{}}0.047***\\ (12.757)\end{tabular}} & \begin{tabular}[c]{@{}l@{}}0.004\\ (0.659)\end{tabular} & \begin{tabular}[c]{@{}l@{}}-0.003\\ (-0.428)\end{tabular} & \begin{tabular}[c]{@{}l@{}}-0.003\\ (-0.447)\end{tabular} & \begin{tabular}[c]{@{}l@{}}-0.005\\ (-0.802)\end{tabular} & 3.01 & cryptocurrency\_risk \\
\multicolumn{1}{l|}{} & \cellcolor[HTML]{EFEFEF}\textbf{\begin{tabular}[c]{@{}l@{}}0.048***\\ (15.22)\end{tabular}} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}0.000\\ (0.065)\end{tabular} & \cellcolor[HTML]{EFEFEF}\textbf{\begin{tabular}[c]{@{}l@{}}-0.010*\\ (-1.938)\end{tabular}} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}-0.001\\ (-0.192)\end{tabular} & \cellcolor[HTML]{EFEFEF}\textbf{\begin{tabular}[c]{@{}l@{}}-0.011**\\ (-2.103)\end{tabular}} & \cellcolor[HTML]{EFEFEF}14.70 & \cellcolor[HTML]{EFEFEF}bitcoin\_attack \\
\multicolumn{1}{l|}{} & \textbf{\begin{tabular}[c]{@{}l@{}}0.046***\\ (14.199)\end{tabular}} & \begin{tabular}[c]{@{}l@{}}0.004\\ (0.503)\end{tabular} & \begin{tabular}[c]{@{}l@{}}-0.003\\ (-0.45)\end{tabular} & \begin{tabular}[c]{@{}l@{}}-0.005\\ (-0.684)\end{tabular} & \begin{tabular}[c]{@{}l@{}}-0.004\\ (-0.519)\end{tabular} & 2.57 & bitcoin\_hack \\
\multicolumn{1}{l|}{} & \cellcolor[HTML]{EFEFEF}\textbf{\begin{tabular}[c]{@{}l@{}}0.048***\\  (15.217)\end{tabular}} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}0.009\\ (1.491)\end{tabular} & \cellcolor[HTML]{EFEFEF}\textbf{\begin{tabular}[c]{@{}l@{}}-0.015**\\ (-2.501)\end{tabular}} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}-0.002\\ (-0.38)\end{tabular} & \cellcolor[HTML]{EFEFEF}\textbf{\begin{tabular}[c]{@{}l@{}}-0.011**\\ (-1.92)\end{tabular}} & \cellcolor[HTML]{EFEFEF}20.8 & \cellcolor[HTML]{EFEFEF}bitcoin\_risk \\
\multicolumn{1}{l|}{} & \textbf{\begin{tabular}[c]{@{}l@{}}0.048***\\ (11.547)\end{tabular}} & \begin{tabular}[c]{@{}l@{}}0.002\\ (0.99)\end{tabular} & \begin{tabular}[c]{@{}l@{}}-0.002\\ (-0.862)\end{tabular} & \begin{tabular}[c]{@{}l@{}}-0.004\\ (-1.577)\end{tabular} & \begin{tabular}[c]{@{}l@{}}-0.001\\ (-0.28)\end{tabular} & 9.42 & bitcoin\_exploit \\
\multicolumn{1}{l|}{} & \cellcolor[HTML]{EFEFEF}\textbf{\begin{tabular}[c]{@{}l@{}}0.046***\\ (12.332)\end{tabular}} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}0.005\\ (0.505)\end{tabular} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}-0.003\\ (-0.352)\end{tabular} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}-0.002\\ (-0.216)\end{tabular} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}-0.009\\  (-0.298)\end{tabular} & \cellcolor[HTML]{EFEFEF}1.26 & \cellcolor[HTML]{EFEFEF}blockchain\_attack \\
\multicolumn{1}{l|}{} & \textbf{\begin{tabular}[c]{@{}l@{}}0.046***\\ (13.282)\end{tabular}} & \begin{tabular}[c]{@{}l@{}}0.003\\ (0.212)\end{tabular} & \begin{tabular}[c]{@{}l@{}}0.002\\ (0.143)\end{tabular} & \begin{tabular}[c]{@{}l@{}}-0.008\\  (-0.58)\end{tabular} & \begin{tabular}[c]{@{}l@{}}-0.009\\ (-0.731)\end{tabular} & 2.02 & blockchain\_hack \\
\multicolumn{1}{l|}{} & \cellcolor[HTML]{EFEFEF}\textbf{\begin{tabular}[c]{@{}l@{}}0.046***\\ (13.994)\end{tabular}} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}-0.008\\ (-0.964)\end{tabular} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}-0.006\\ (-0.769)\end{tabular} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}0.003\\ (0.27)\end{tabular} & \cellcolor[HTML]{EFEFEF}\begin{tabular}[c]{@{}l@{}}0.002\\ (0.246)\end{tabular} & \cellcolor[HTML]{EFEFEF}3.93 & \cellcolor[HTML]{EFEFEF}blockchain\_risk \\
\multicolumn{1}{l|}{\multirow{-11}{*}{\textit{\textbf{BTC volatility}}}} & \textbf{\begin{tabular}[c]{@{}l@{}}0.043***\\ (12.069)\end{tabular}} & \begin{tabular}[c]{@{}l@{}}0.003\\  (0.548)\end{tabular} & \begin{tabular}[c]{@{}l@{}}0.007\\ (1.149)\end{tabular} & \begin{tabular}[c]{@{}l@{}}0.006\\ (1.033)\end{tabular} & \begin{tabular}[c]{@{}l@{}}0.003\\ (0.534)\end{tabular} & 3.60 & Zero-day \\ \hline
\end{tabular}}
			\\ \footnotesize{\textit{Here we assess the impact of negative investor’s attention, proxied by google search terms “cryptocurrency attack/hack /risk”, ”bitcoin attack/hack/risk/", “blockchain attack/hack/risk” and "zero day". We regressed bitcoin's monthly volatility against the google search data without lag and lagged from one up to three-months horizons. For each regression, we had 50 observations. *** denotes significance levels based on the respective p-value (*:10\%, **:5\%, and ***:1\%). The standard t-statistic value is shown in parentheses.}	}
}	\end{table}


To assess the negative investor attention’s impact on bitcoin's performance, we regress the monthly bitcoin's return (volatility) against google search volumes without lag and lagged from one up to three months. Considering that financial markets usually experience instability episodes around uncertain times (negative events) \citep{Acharya2017, Gray2007, Palma-Ruiz2020, Maouchi2021, Yousaf2022}, we expect that our results will show a decrease (increase) in bitcoin's returns (volatility) as a response to significant negative investor attention. \\

Out findings show that Google searches have a significant impact on bitcoin's returns, which first of all proves that negatives investor attention is captured by GSV. Furthermore, we observe that for most of the phrases used, the effect is persistent, impacting bitcoin's returns up to two months. Although, the relationship between bitcoin's returns and investors’ negative attention seems to be the strongest for lag = 0, which indicates that the reaction of investors’ attention might not impact returns in the long run. At the same time, our non-specialist considered keywords seem to drive returns more than the ones of a Blockchain specialist. This result suggests that the majority of crypto-investors are in fact amateur or non-specialists in the crypto-market. This evidence is in line with the existing literature \citep{Hasso2019, Lucey2022}.  \\

Now, when assessing if negative investor attention impacts bitcoin's volatility, we have interesting results. With only ‘bitcoin attack’ and ‘bitcoin risk’ searches being significant, our evidences show again that the non-specialist investors are the ones impacting the stability in the crypto-market. Moreover, according to our findings, the negative investor attention tends to decrease the volatility in the coming months. This result is surprising and contradicts our initial assumptions, therefore we conclude that further investigations would be necessary to clarify this issue. \\


Our results have important implications for the researchers and (potential) investors interested in the crypto-market. Among its many interesting features and innovative advantages, Blockchain technology and the crypto-market are mostly famous thanks to the highly volatile crypto-assets' prices \citep{Yi2018, Dyhrberg2016, Baur2021}. Our findings bring new evidence that could help to better understand this market. At the same time, we think that supporting Blockchain literacy among investors, would greatly improve the performance and reputation of the crypto-market as a whole.


\section{Conclusion}

The crypto-market emerged in 2008, together with the first crypto-asset created, bitcoin. Since then, Blockchain technology has evolved, potentially disrupting many fields beyond finance. However, still in infancy compared to its promised future, the crypto market has to overcome its many challenges. We believe that understanding and analyzing the crypto-market vulnerabilities represents the first step in overcoming its challenges. \\

In this paper, we perform a literature survey concentrating on the types of risks present in the crypto-market. Our focus is on the technological and financial risks of the crypto-market and Blockchain technology. First, we show that these risks can be related and that during specific market conditions, they can become a trigger one for another. Second, we offer a way to determine the likelihood of triggering financial risks through technological vulnerabilities. Here, we also emphasize the role played by financial behavior, social responsibility, and Blockchain literacy in the stability of the crypto-market. Furthermore, to complete this study, we perform a short data analysis, demonstrating that crypto-assets’ price stability can be disrupted by technological vulnerabilities characteristic of this market. More research is needed on this matter, however, with the little data available, we showed that bitcoin's volatility is influenced by the number of events targeting it. This evidence reveals the implication of cybersecurity risks and poor regulation in the crypto-market development. \\
Given that bitcoin's volatility is impacted by technological vulnerabilities, we pushed this investigation one step further. Complementary to the first analysis, we explored the impact of negative investor attention on bitcoin's returns and volatility. Our findings show that bitcoin's performance is mostly impacted by the non-specialist investors (amateurs) who trade crypto-assets. In accordance with our initial assumption, bitcoin returns are influenced by the negative investor attention. We think that an increase in Blockchain literacy among the crypto-investors could greatly help this market to perform better and improve its image.  \\
...
We think this paper may be helpful for both academic researchers in their efforts to understand the determinants of the crypto-assets risk and to market participants (as well as crypto-asset enthusiasts) for their investments.  \\
...
\rowcolor[HTML]{FFFFFF}
9/15/2015 & 5,000  & \textit{BitPay phishing scam - hacker takes over the CEO's accounts} \\
\rowcolor[HTML]{FFFFFF}
...
\rowcolor[HTML]{FFFFFF}
7/12/2017     & 1,942                      & \textit{AlphaBay (darknet) admins assets sized by FBI}  \\
\rowcolor[HTML]{FFFFFF}
7/20/2017     & 1,200                      & \textit{Hansa (darknet) funds seized by Dutch police}  \\
\rowcolor[HTML]{FFFFFF}
