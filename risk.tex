...
twofold. First, we show a link between the technological risks and financial ones. Secondly, we find that price
...
\footnote{Specific risk concerns isolated cases (one crypto-asset or a
 ...
 Finally, we can observe that in spite of its technological nature and distinct vulnerabilities, the crypto-market, similar to financial traditional market, is susceptible to same financial risks, namely systemic, systematic and specific risks. 
...
way the market evolves \citep{Zhao2021}. Market signals can be complex, including both
information and noise \citep{Rizzi2008}.
...
transact/trade. Liquidity risk, can be split into three categories: assets liquidity
 ...
 size of the network \citep{Koutmos2018}. According to \cite{Corbet2019}, liquidity risk is also highly
...
It is important to mention the fact that liquidity is different from one
cryptocurrency to another (the well established ones are more liquid \citep{Wei2018}), as
...
present some advantages, especially for the traders on this market,
which can benefit from arbitrage opportunities and purchases at
...
financial markets (it is one of the determinants for market efficiency), we
...
\citep{Manahov2020}. A proof of market efficiency is the difficulty to manipulate prices. In the crypto-market, specificallyconcerning bitcoin, it has been observed a significant herding
...
environmental and social implications, it is believed that a more
ecologically oriented Blockchain could significantly change the
overall ‘safety’ perception \citep{Lai2021}. If this kind of risk doesn’t have direct
...
In line with the literature survey done in the previous sections, here we are going to provide an example of how financial risk is linked to technological vulnerabilities. More specifically, we are going to
assess if bitcoin’s volatility is affected by the events targeting the crypto-market. A preliminary work on this problem has already been done by \cite{Grobys2021}. The author shows how the hackings performed on bitcoin during the 2013–2017 period, affected BTC and ETH returns. In our analysis, we are using a bigger sample of events, while at the same time referring to a longer period of time: 2011 - 2018. 

This data analysis is an illustration meant to complement the survey performed. In accordance with the literature and with the aim to answer to our research question: \textit{'Can financial risks be triggered by technological vulnerabilities of Blockchain technology?'}, we establish the following hypotheses:\\
...
\begin{table}[h]
	...
	\label{tab:descriptiveStatistics}
		\begin{tabular}{lrrr}
			\toprule
			 & vol & event losses & number of events \\
			\cmidrule[0.4pt]{1-4}
			Valid & 27 & 27 & 27  \\
			Missing & 0 & 0 & 0  \\
...
\begin{itemize}
\item Cryptocurrencies represent a virtual currency; built on open source software code, they exist and operate just in the online environment. This make them the target of cyberattacks that try to exploit any possible vulnerability of this technology.
...
Our regression checks if there is a relationship between the monthly volatility of bitcoin and the monthly number of events with their respective losses. For the number of events, we obtain a $\beta$ of 0.193 with p-value equal to 0.026. Meanwhile, for the losses incurred during these events, we obtain a $\beta$ of -7.589e-8 and p-value equal to 0.832. We therefore conclude that our sample data provided enough evidence to show that there is a
...
This paper is an academic one addressed mainly to researchers, but which should be of great interest to both market participants and cryptocurrency enthusiasts.  \\
