...
twofold. First, we show an identified continuity\todoJGD{link?} between the
...
transaction” \footnote{Own translation from original: ” Internet a
  ...
  transaction” \citep{Charles2019}\todoJGD{cite Beddiar's book instead}}, however, there is still a lot of
...
\todoJGD{No we do not ``claim'', we propose something different or not
  yet done}
...
knowledge. 
\todoJGD{No we do not believe, we follow that and this papers ...}
In conducting this research, we have used various types of
...
\todoJGD{give a partial answer now, the rest of the paper being a
...
political issues.
\todoJGD{we believe ? do we have a proof of these two previous
  sentences ?}
 That being said, in this study, we tackle the first
...
\todoJGD{Rizzi2008 not defined}
 Less mysterious than at the
...
\todoJGD{we beleive ? any proof or citation ?}
 If this kind of risk doesn’t have direct
...
$$ \sigma = \alpha_n + \beta_n * EVENT_{number} + \epsilon_n   $$
$$ \sigma = \alpha_a + \beta_a * EVENT_{amount} + \epsilon_a   $$
...
results obtained are significant, with a p value of 0.0216.
\todoJGD{define p}
 We
...
\todoJGD{we argue ? is it a fact or are there evidence?}
...
Overall, we think this analysis could also be useful not only to the existing
participants but to those considering to enter this 
market as well, them being practitioners or academics. \\
...
\appendix
\section{Appendix}
\todoJGD{provide me here (in the captions?)  with means to reproduce these tables for myself}
\begin{table}[ht]
\centering
